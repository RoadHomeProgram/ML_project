...
RCIdata<-RCIdata %>% mutate(OutcomeRCscore=reliablechangescore) %>% dplyr::select(-reliablechangescore)
colnames(RCIdata)
outcomes<-colnames(RCIdata)[grepl("Outcome",colnames(RCIdata))]
features<-colnames(RCIdata)[!grepl("Outcome",colnames(RCIdata))]                
```
...
estimateModelPerformance<-function(data,outcomeStr,k=5){
  ...
  for (fold in 1:k){
    ...
    prediction_list[[fold]]<-prediction_matrix
  }
...
```{r}
for (i in outcomes){
  input<-generate_dataset(RCIdata,featureVec = features,i)
